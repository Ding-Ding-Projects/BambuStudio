...
PartSkipDialog::PartSkipDialog(wxWindow *parent) : DPIDialog(parent, wxID_ANY, _L("Skip Objects"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    m_all_label->SetMinSize(FromDIP(wxSize(267, -1)));
    m_all_label->SetMaxSize(FromDIP(wxSize(267, -1)));
...
std::string PartSkipDialog::create_tmp_path()
{
    ...
    if (!std::filesystem::exists(tmp_path + "Metadata/") && !fs::create_directories(tmp_path + "Metadata/")) { MessageDialog dlg(this, "create file failed."); dlg.ShowModal(); }
    return tmp_path;
}
