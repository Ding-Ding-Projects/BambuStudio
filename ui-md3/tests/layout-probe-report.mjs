...
for (const file of files) {
  ...
  const shownChain = (w) => {
    ...
    // A dialog owned by the frame records the frame as its top, so the
    // top-level flag alone misses a hidden dialog: any hidden ancestor on the
    // parent chain hides the window, whatever on_screen says.
