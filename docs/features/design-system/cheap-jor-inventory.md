...
| CJ-007 | Prepare action bar, Slice / Print options segments | every tuple | the 24 px options segment shows as a bare green sliver two pixels short of its minimum, no chevron | the segment had no content once the raster caret was dropped; SideButton measured it below the sizer allocation | bb4a0988a | prepare--en-light-comfortable--before.png | pending | fixed-unverified |
| CJ-008 | Prepare ink rows, colour swatch button | every tuple (first after-build capture) | swatch widened to 44 px and painted a second "1" beside the badge | Plater set the swatch label to the filament index as an accessibility hack; the kit Button paints labels | bb4a0988a | evidence/ink-row-swatch-label--bbc9db5cf.png | pending | fixed-unverified |
| CJ-009 | Home tab, window caption bar | every tuple, first capture after a 1000 to 1200 px resize | the caption bar (menus, project chip, window controls) stays 792 px wide on a 1186 px client; the window controls sit mid-window and the rest of the strip is bare | under investigation: the frame forces the top bar width on every size event, so the stale paint points at a missing refresh or a layout skipped on the Home page; needs a runtime repro on attempt 9 | pending | home--en-light-comfortable--before.png | pending | open |
